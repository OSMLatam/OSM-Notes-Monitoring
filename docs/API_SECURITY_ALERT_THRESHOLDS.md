...
This document defines all alert thresholds for the OSM-Notes-API security component. These thresholds are used to trigger alerts when security metrics exceed acceptable levels.
...
- **Description:** Maximum number of requests per second from a single IP before triggering DDoS detection
...
- **Default Value:** `500`
- **Unit:** Connections
- **Description:** Maximum number of concurrent connections from unique IPs before triggering DDoS detection
...
- **Default Value:** `50`
- **Unit:** Percent
- **Description:** Maximum percentage of requests that result in errors (4xx/5xx) before triggering abuse detection
