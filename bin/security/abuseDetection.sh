...
analyze_patterns() {
    ...
    if [[ "${abuse_detected}" == "true" ]]; then
        local reason_str
        reason_str=$(IFS=','; echo "${abuse_reasons[*]}")
        log_warning "${COMPONENT}: Abuse pattern detected for ${ip}: ${reason_str}"
        record_security_event "abuse" "${ip}" "" "{\"patterns\": [$(IFS=','; printf '"%s"' "${abuse_reasons[@]}")], \"rapid_count\": ${rapid_count}, \"error_rate\": ${error_rate}, \"excessive_count\": ${excessive_count}}"
...
analyze_behavior() {
    ...
    if [[ "${suspicious}" == "true" ]]; then
        local reason_str
        reason_str=$(IFS=','; echo "${behavior_reasons[*]}")
        log_warning "${COMPONENT}: Suspicious behavior detected for ${ip}: ${reason_str}"
        record_security_event "abuse" "${ip}" "" "{\"type\": \"behavioral\", \"reasons\": [$(IFS=','; printf '"%s"' "${behavior_reasons[@]}")], \"endpoint_count\": ${endpoint_count}, \"ua_count\": ${ua_count}}"
